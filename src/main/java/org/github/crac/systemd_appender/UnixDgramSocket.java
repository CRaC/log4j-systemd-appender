...
/**
 * Thin JNI wrapper around a single AF_UNIX SOCK_DGRAM file descriptor.
 * All journal protocol encoding is done in Java; this class only handles
 * the OS-level socket operations.
 */
final class UnixDgramSocket implements AutoCloseable {
    ...
    UnixDgramSocket() throws IOException {
        int result = open0();
        if (result < 0) {
